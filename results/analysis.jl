...
function main()
    ...
    for name in names
        if isfile("$(name)_data.json")
            ...
            p = plot(title="Runtime vs. Dimension (Density p=0.01)", xlabel="Dimension n", ylabel="Runtime (Seconds)", xscale=:log10, yscale=:log10)
            ...
            p = plot(title="Runtime vs. Density (Dimension n=10^4)", xlabel="Density p", ylabel="Runtime (Seconds)", xscale=:log10, yscale=:log10)
