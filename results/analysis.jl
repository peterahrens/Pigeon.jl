...
function main()
    ...
    for name in names
        if isfile("$(name)_data.json")
            ...
            p = plot(title="Runtime vs. Dimension (p=0.01)", xlabel="Dimension n", ylabel="Runtime (Seconds)", xscale=:log10, yscale=:log10, legend=:topleft, titlefontsize=10, thickness_scaling=1.7)
            ...
            p = plot(title="Runtime vs. Density (n=$(data["N"]))", xlabel="Density p", ylabel="Runtime (Seconds)", xscale=:log10, yscale=:log10, legend=:topleft, titlefontsize=10, thickness_scaling=1.7, xflip=false)
